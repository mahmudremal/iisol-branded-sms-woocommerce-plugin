...
class Dashboard {
  ...
  public function admin_menu() {
    add_menu_page(
      __(  'CRM Dashboard', 'iisol-branded-sms-woocommerce-plugin' ),     // page title
      __(  'Dashboard', 'iisol-branded-sms-woocommerce-plugin' ),     // menu title
      'manage_options',   // capability
      'crm_dashboard',     // menu slug  
      [ $this, 'crm_dashboard' ], // callback function
      FUTUREWORDPRESS_PROJECT_BRANDEDSMS_BUILD_URI . '/icons/dashboard-alt.svg',
      10
    );
  }
  public function crm_dashboard() {
    ...
    <div class="wrap dashboard__body-start">
      <?php include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/crm_dashboard.php'; ?>
    </div>
    <?php
  }
  public function wp_after_admin_bar_render() {
    if( ! isset( $_GET[ 'page' ] ) || ! in_array( $_GET[ 'page' ], $this->allowedPages ) ) {return;}
    include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/bar_render.php';
  }
  ...
  public function pageTree( $args ) {
    return [
      ...
      'leads'     => [
        'title'   => __(  'Leads from Facebook', 'iisol-branded-sms-woocommerce-plugin' ),
        ...
        'submenu' => [
          // 'spages' => [
          //   'title'   => __(  'pages', 'iisol-branded-sms-woocommerce-plugin' ),
          //   'icon'    => '<svg class="icon-20" width="20" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M9.14373 20.7821V17.7152C9.14372 16.9381 9.77567 16.3067 10.5584 16.3018H13.4326C14.2189 16.3018 14.8563 16.9346 14.8563 17.7152V20.7732C14.8562 21.4473 15.404 21.9951 16.0829 22H18.0438C18.9596 22.0023 19.8388 21.6428 20.4872 21.0007C21.1356 20.3586 21.5 19.4868 21.5 18.5775V9.86585C21.5 9.13139 21.1721 8.43471 20.6046 7.9635L13.943 2.67427C12.7785 1.74912 11.1154 1.77901 9.98539 2.74538L3.46701 7.9635C2.87274 8.42082 2.51755 9.11956 2.5 9.86585V18.5686C2.5 20.4637 4.04738 22 5.95617 22H7.87229C8.19917 22.0023 8.51349 21.8751 8.74547 21.6464C8.97746 21.4178 9.10793 21.1067 9.10792 20.7821H9.14373Z" fill="currentColor"></path></svg>',
          //   'submenu' => []
          // ],
        ]
      ...
      // 'payments' => [
      //   'title'   => __(  'Payments', 'iisol-branded-sms-woocommerce-plugin' ),
      ...
      //   'submenu' => [
      //     'stripe_logs' => [
      //       'title'   => __(  'Stripe logs', 'iisol-branded-sms-woocommerce-plugin' ),
      //       'submenu' => []
      //     ],
      //   ]
      // ],
      'archives' => [
        'title'   => __(  'Archives', 'iisol-branded-sms-woocommerce-plugin' ),
  ...
  public function partsCall( $position ) {
    ...
    switch( $position ) {
      case 'after_nav':
        include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/after_nav.php';
        break;
      case 'content':
        if( in_array( 'dashboard', [ $current_pathinfo[0], '' ] ) ) {
          include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/content.php';
        } else {
      ...
      case 'homecontent':
        if( in_array( 'dashboard', [ $current_pathinfo[0], '' ] ) ) {
          // include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/tasks.php';
          include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/leads.php';
        } else {
          do_action( 'futurewordpress/project/parts/split', $this->args );
        }
        break;
  ...
  public function partSplit( $args ) {
    if( $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] == 'edit' || $args[ 'split' ][1] == 'add' ) {
      include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/editprofile.php';
    } else if( $args[ 'split' ][0] == '' || $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] === false ) {
      include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/leads.php';
    } else if( $args[ 'split' ][0] == 'payments' && $args[ 'split' ][1] == false ) {
      include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/stripe.php';
    } else if( $args[ 'split' ][1] == 'stripe_logs' ) {
      include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/' . ( ( $args[ 'split' ][2] === false ) ? 'stripe_logs' : 'stripe_details' ) . '.php';
    } else if( $args[ 'split' ][0] == 'archives' ) {
      include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/' . ( ( $args[ 'split' ][2] === false ) ? 'archives' : 'archive' ) . '.php';
    } else if( $args[ 'split' ][0] == 'notices' ) {
      include FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/admin/notices.php';
    } else {}
  }
  public function adminTitle( $default, $args ) {
    ...
    if( $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] == 'edit' || $args[ 'split' ][1] == 'add' ) {
      return __( 'Edit Lead', 'iisol-branded-sms-woocommerce-plugin' );
    } else if( $args[ 'split' ][0] == '' || $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] === false ) {
      return __( 'Leads', 'iisol-branded-sms-woocommerce-plugin' );
    } else if( $args[ 'split' ][0] == 'payments' && $args[ 'split' ][1] == false ) {
      return __( 'Payments', 'iisol-branded-sms-woocommerce-plugin' );
    } else if( $args[ 'split' ][1] == 'stripe_logs' ) {
      return __( 'Stripe Logs', 'iisol-branded-sms-woocommerce-plugin' );
    } else if( $args[ 'split' ][0] == 'archives' ) {
      return __( 'Archive Uploaded', 'iisol-branded-sms-woocommerce-plugin' );
    } else if( $args[ 'split' ][0] == 'notices' ) {
      return __( 'Notices', 'iisol-branded-sms-woocommerce-plugin' );
    } else {return  $default;}
  }
