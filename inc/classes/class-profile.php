...
class Profile {
	...
	public function uploadAvater() {
		...
		if( $upload_dir && move_uploaded_file( $_FILES[ 'avater' ][ 'tmp_name' ], $upload_dir . '/' . $newFileName ) ) {
			...
			wp_send_json_success( __( 'Profile Image Updated Successfully.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		} else {
			wp_send_json_error( __( 'Profile Image upload failed.', 'iisol-branded-sms-woocommerce-plugin' ) );
		}
	}
