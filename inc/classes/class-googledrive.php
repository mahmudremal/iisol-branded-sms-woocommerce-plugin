...
class GoogleDrive {
	...
	public function developmentMode() {
		...
		try{
			...
			// $drive_file_move = $this->uploadFileWithMeta( FUTUREWORDPRESS_PROJECT_BRANDEDSMS_BUILD_PATH . '/img/img-to-contract.jpg' );
	...
	public function submitArchives() {
		...
		if( $result ) {
			...
			wp_send_json_success( [ 'message' => __( 'Archived Successfully.', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => [ 'reload-page' ] ], 200 );
		} else {
			wp_send_json_error( __( 'Problem detected while creating archive.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function deleteArchives() {
		...
		wp_send_json_success( [ 'message' => __( 'Archive removed from server successfully!', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => [ 'reload-page' ] ], 200 );
	}
    ...
    if ($http_code == 204) {
			...
			$error_msg = ( $http_code == 404 ) ? __( 'File Not found', 'iisol-branded-sms-woocommerce-plugin' ) : __( 'Failed to delete file from drive.', 'iisol-branded-sms-woocommerce-plugin' );
			throw new \Exception('Error '.$http_code.': ' . $error_msg );
    }
	}
	public function createDirectory( $folderName ) {
		...
		$response = json_decode( $response, true );
		// print_r( $response );
		$error_msg = __( 'Error creating folder:', 'iisol-branded-sms-woocommerce-plugin' ) . ( ! empty( curl_error( $curl ) ) ? ': ' . curl_error( $curl ) : '' );
		curl_close($curl);
		if (curl_errno($curl)) {
