...
class Shortcode {
	...
	public function checkout_video( $args ) {
		...
		if( ! $args[ 'post_id' ] || empty( $args[ 'post_id' ] ) ) {$errorHappens = __( 'Link that required video clip ID included.', 'iisol-branded-sms-woocommerce-plugin' );}
		$meta = get_post_meta( $args[ 'post_id' ], 'checkout_video_clip', true );
		if( ! isset( $meta[ 'full_path' ] ) || $meta[ 'full_path' ] === false || ! file_exists( $meta[ 'full_path' ] ) || is_dir( $meta[ 'full_path' ] ) ) {$errorHappens = __( 'Video file not found. Maybe expired :( If so, message is file removed permanently from server.', 'iisol-branded-sms-woocommerce-plugin' );}
		// if( ! is_user_logged_in() || get_current_user_id() != get_post_field( 'post_author', $args[ 'post_id' ] ) || ! current_user_can( 'manage_options' ) ) {$errorHappens = __( 'You don\'t have authorization to access this video. Only people who placed order or uploaded video and admin can access this video.', 'iisol-branded-sms-woocommerce-plugin' );}
		wp_enqueue_script( 'FutureWordPressProjectBrandedSms' );wp_enqueue_script( 'FutureWordPressProjectBrandedSms-checkout' );
		ob_start();
			...
			<div class="fwp-video-wrap">
				<video playsinline class="video-js vjs-default-skin fwp-videojs-playing-field" controls preload="auto" data-temp-poster="" data-setup='{ "controls": true, "autoplay": false, "preload": "none", "fluid": true, "fill": true, "responsive": true }'>
					...
					<p class="vjs-no-js">
						...
						web browser that', 'iisol-branded-sms-woocommerce-plugin' ); ?>
						<a href="https://videojs.com/html5-video-support/" target="_blank">
							<?php esc_html_e( 'supports HTML5 video', 'iisol-branded-sms-woocommerce-plugin' ); ?>
						</a>
					</p>
				</video>
			</div>
