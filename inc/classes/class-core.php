...
class Core {
	...
	public function requestDashboard() {
		...
		if( ! wp_verify_nonce( $_POST['_nonce'], 'futurewordpress/project/nonce/dashboard' ) ) {
			wp_die( __( 'Nonce doesn\'t matched from your request. if you requested from an expired form, please do a re-submit', 'iisol-branded-sms-woocommerce-plugin' ), __( 'Security verification mismatched.', 'iisol-branded-sms-woocommerce-plugin' ) );
		}
		...
		set_transient( 'futurewordpress/project/transiant/admin/' . get_current_user_id(), [
			...
			'message'				=> __( 'Request detected but is staging mode.', 'iisol-branded-sms-woocommerce-plugin' )
		], 200 );
	...
	public function singleField() {
		if( ! isset( $_POST[ 'field' ] ) || ! isset( $_POST[ 'value' ] ) || ! isset( $_POST[ '_nonce' ] ) || ! wp_verify_nonce( $_POST[ '_nonce' ], 'futurewordpress/project/verify/nonce' ) ) {
			wp_send_json_error( __( 'We\'ve detected you\'re requesting with an invalid security token or something went wrong with you', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
		...
		if( ! empty( $field ) && isset( $_POST[ 'userid' ] ) ) {
			...
			if( $type = 'meta' && array_key_exists( $field, $userMeta ) ) {
				if( $field == 'enable_subscription' ) {
					...
					wp_send_json_success( __( 'Profile Information Update successful', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
				}
				...
				if( in_array( $field, [ 'display_name', 'user_email' ] ) ) {
					...
					wp_send_json_success( __( 'Profile Update successful', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
				} else {
					wp_send_json_error( __( 'Illigal request sent. Nothing happen. Request rejected.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
				}
			} else {
				wp_send_json_error( __( 'Request properly not identified or not allowed to madify.', 'iisol-branded-sms-woocommerce-plugin' ) . $field, 200 );
			}
		} else {
			wp_send_json_error( __( 'Failed operation', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function selectToRegister() {
		...
		if( $user_id && $reg_key && $reg_type ) {
			...
			wp_send_json_success( [
				'message'			=> __( 'Successfully Saved your choice. Current page should reload not. If you don\'t see, page not reloading, please reload this page.', 'iisol-branded-sms-woocommerce-plugin' ),
				'hooks'				=> [ 'reload-page' ]
			], 200 );
		} else {
			wp_send_json_error( __( 'Technical Error. If you saw this message, please contact with site administrative.', 'iisol-branded-sms-woocommerce-plugin' ) );
		}
	}
	public function actionStatuses( $args, $specific = false ) {
		$actions = [
			'call_scheduled'            => __( 'Step 1.Call Scheduled', 'iisol-branded-sms-woocommerce-plugin' ),
			'no_show'                   => __( 'Step 2a. No Show', 'iisol-branded-sms-woocommerce-plugin' ),
			'call_rescheduled'          => __( 'Step 2b. Call Rescheduled', 'iisol-branded-sms-woocommerce-plugin' ),
			'setfollowup'            		=> __( 'Step 3a. Set Follow Up', 'iisol-branded-sms-woocommerce-plugin' ),
			'send_contract'            	=> __( 'Step 3b. Send Contract', 'iisol-branded-sms-woocommerce-plugin' ),
			'contract_pending'					=> __( 'Step 4. Contract Pending', 'iisol-branded-sms-woocommerce-plugin' ),
			'payment_pending'           => __( 'Step 5. Payment Pending', 'iisol-branded-sms-woocommerce-plugin' ),
			'payment_confirmed'					=> __( 'Step 6. Payment Confirmed', 'iisol-branded-sms-woocommerce-plugin' ),
			'retainer_scheduled'        => __( 'Step 7. Retainer Scheduled', 'iisol-branded-sms-woocommerce-plugin' ),
			'payment_issues'            => __( 'Step 8. Payment Issues', 'iisol-branded-sms-woocommerce-plugin' ),
			'refund_scheduled'          => __( 'Step 9. Refund Scheduled', 'iisol-branded-sms-woocommerce-plugin' ),
			'retainer_cancelled'        => __( 'Step 10. Retainer Cancelled', 'iisol-branded-sms-woocommerce-plugin' ),
			'other'           					=> __( 'Other', 'iisol-branded-sms-woocommerce-plugin' ),
		];
	...
	public function editSubscriber() {
		if( ! isset( $_POST[ '_nonce' ] ) || ! wp_verify_nonce( $_POST[ '_nonce' ], 'futurewordpress/project/nonce/editsubscriber' ) ) {
			wp_die( __( 'We\'ve detected you\'re requesting with an invalid security token or something went wrong with you', 'iisol-branded-sms-woocommerce-plugin' ), __( 'Security mismatched', 'iisol-branded-sms-woocommerce-plugin' ) );
		}
		...
		if( isset( $_POST[ 'userdata' ] ) ) {
			...
			if ( ! is_wp_error( $is_created ) ) {
				$msg = [
					...
					'message'				=> __( 'User Information has been successfully updated.', 'iisol-branded-sms-woocommerce-plugin' )
				];
			} else {
				$errormessage = $is_created->get_error_message();
				$msg = [
					'type'					=> 'warning', // primary | danger | success | warning | info
					'message'				=> ( empty( $errormessage ) ) ? __( 'Failed to update user information.', 'iisol-branded-sms-woocommerce-plugin' ) : $errormessage
				];
			}
	...
	public function switchLeadStatus() {
		...
		if( empty( $this->actionStatuses( [], $_POST[ 'value' ] ) ) ) {
			wp_send_json_error( __( 'Unexpected status requested.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
		if( isset( $_POST[ 'lead' ] ) && ! empty( $_POST[ 'value' ] ) ) {
			update_user_meta( $_POST[ 'lead' ], 'status', $_POST[ 'value' ] );
			wp_send_json_success( [ 'message' => __( 'Updated Successfully.', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => ['lead-status-' . $_POST[ 'lead' ] . '-' . $_POST[ 'value' ] ] ], 200 );
		} else {
			wp_send_json_error( __( 'Status request contains empty arguments.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function deleteLeadAccount() {
		...
		if( isset( $_POST[ 'lead' ] ) && ! empty( $_POST[ 'lead' ] ) ) {
			...
			wp_send_json_error( [ 'message' => __( 'Deleted User Successfully.', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => ['delete-lead-' . $_POST[ 'lead' ] ] ], 200 );
		} else {
			wp_send_json_error( __( 'Unexpected status requested.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function deletePayment() {
		...
		if( isset( $_POST[ 'id' ] ) && ! empty( $_POST[ 'id' ] ) ) {global $wpdb;
			...
			if( $is_done && ! is_wp_error( $is_done ) ) {
				wp_send_json_success( [ 'message' => __( 'Deleted Payment Log Successfully.', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => ['delete-stripe-log-' . $_POST[ 'id' ] ] ], 200 );
			} else {
				wp_send_json_error( [ 'message' => is_wp_error( $is_done )? $is_done->get_error_message() : __( 'Failed to delete payment log.', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => [] ], 200 );
			}
		} else {
			wp_send_json_error( __( 'Unexpected status requested.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	...
	public function deleteNotices() {
		...
		if( isset( $_POST[ 'delete' ] ) && ! empty( $_POST[ 'delete' ] ) ) {
			...
			wp_send_json_success( __( 'All notices deleted successfully.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		} else {
			wp_send_json_error( __( 'Unexpected status requested.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
    ...
    $lead_registration = get_query_var( 'lead_registration' );// $order_id = get_query_var( 'order_id' );
      ...
      return $template;
			...
			$file = FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/cards/lead_registration.php';
	...
	public function sendRegistration() {
		...
		if( isset( $_POST[ 'lead' ] ) && ! empty( $_POST[ 'lead' ] ) ) {
			...
			if( $userInfo->meta->monthly_retainer <= 0 ) {
				wp_send_json_error( __( 'User\'s retainer amount is zero, you\'ve change retainer amount to send registration link.', 'iisol-branded-sms-woocommerce-plugin' ) );
			}
			...
			if( apply_filters( 'futurewordpress/project/mailsystem/sendmail', $args ) ) {
				wp_send_json_success( [ 'message' => __( 'Registration Link sent successfully!', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => [ 'sent-registration-' . $_POST[ 'lead' ] ] ], 200 );
			} else {
				wp_send_json_error( __( 'Mail not sent.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
			}
		}
		wp_send_json_error( __( 'Unexpected status requested.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
	}
	public function sendPasswordReset() {
		...
		$userMeta = array_map( function( $a ){ return $a[0]; }, (array) get_user_meta( $userInfo->ID ) );
		$userInfo = (object) wp_parse_args( $userInfo, [ 'id' => '', 'meta' => (object) wp_parse_args( $userMeta, apply_filters( 'futurewordpress/project/usermeta/defaults', (array) $userMeta ) ) ] );
		if ( ! $userInfo ) {wp_send_json_error( __( 'User doesn\'t identified.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );}
		...
		if ( apply_filters( 'futurewordpress/project/mailsystem/sendmail', $args ) ) {
			wp_send_json_success( [ 'message' => __( 'Reset Link sent successfully!', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => [ 'sent-passreset-' . $_POST[ 'lead' ] ] ], 200 );
		} else {
			wp_send_json_error( __( 'Unexpected respond from backend.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function toggleSubscption( $user_id, $meta_key, $meta_value ) {
		...
		if( $meta_value == 'off' && ! apply_filters( 'futurewordpress/project/payment/stripe/allowswitchpause', true, 'pause', $userInfo->ID ) ) {
			wp_send_json_success( [ 'message' => __( 'You can\'t change now. You can only pause you retainer once every 60 days. Please wait until it release.', 'iisol-branded-sms-woocommerce-plugin' ), 'hooks' => [] ], 200 );
		} else {
			...
			if( apply_filters( 'futurewordpress/project/payment/stripe/subscriptionToggle', $status, ( ! empty( $userInfo->data->user_email ) ? $userInfo->data->user_email : get_user_meta( $user_id, 'email', true ) ), $user_id ) ) {
				...
				$notice = apply_filters( 'futurewordpress/project/notices/manager', 'add', 'cancelSubscription', [
					'type'						=> 'warn',
					'message'					=> sprintf( __( '%s %s his Subscription', 'iisol-branded-sms-woocommerce-plugin' ), '<a href="' . admin_url( 'admin.php?page=crm_dashboard&path=leads/edit/' . $userInfo->ID . '/' ) . '" target="_blank">' . get_user_meta( $userInfo->ID, 'first_name', true ) . ' ' . get_user_meta( $userInfo->ID, 'last_name', true ) . '</a>', strtoupper( $status ) ),
					'data'						=> [
						'time'					=> time(),
				...
				// 'message' => __( 'User subscription Changed Successfully', 'iisol-branded-sms-woocommerce-plugin' ), 
				wp_send_json_success( [ 'hooks' => ['subscription-status-' . $status ] ], 200 );
			} else {
				wp_send_json_error( __( 'Failed to Switch subscription! Please contact with administrative for assistance.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
			}
		}
	...
	public function cancelSubscription() {
		if( ! apply_filters( 'futurewordpress/project/system/isactive', 'stripe-cancelsubscription' ) ) {
			wp_send_json_error( __( 'Subscription calcelletion is not allowed from administrative. Please contract with them manually.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
		...
		if( $userInfo && apply_filters( 'futurewordpress/project/payment/stripe/subscriptionCancel', 'cancel', ( ! empty( $userInfo->data->user_email ) ? $userInfo->data->user_email : get_user_meta( $userInfo->ID, 'email', true ) ), $userInfo->ID ) ) {
			...
			$notice = apply_filters( 'futurewordpress/project/notices/manager', 'add', 'cancelSubscription', [
				'type'						=> 'alert',
				'message'					=> sprintf( __( 'An User (%s) cancelled Subscription on %s', 'iisol-branded-sms-woocommerce-plugin' ), '<a href="' . admin_url( 'admin.php?page=crm_dashboard&path=leads/edit/' . $userInfo->ID . '/' ) . '" target="_blank">' . get_user_meta( $userInfo->ID, 'first_name', true ) . ' ' . get_user_meta( $userInfo->ID, 'last_name', true ) . '</a>', wp_date( 'M, d H;i' ) ),
				'data'						=> [
					'time'					=> wp_date( 'Y-M-d H:i:s' ),
					'user'					=> $_GET[ 'userid' ]
				]
			] );
			wp_send_json_success( __( 'You successfully Cancelled your subscriptions. A notification to was sent to the Administrative.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		} else {
			wp_send_json_error( __( 'Subscription calcelletion failed. Please contact with site administrative for further assistance.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function switchPayementCard() {
		if( ! isset( $_POST[ '_nonce' ] ) || ! wp_verify_nonce( $_POST[ '_nonce' ], 'futurewordpress/project/verify/nonce' ) ) {
			wp_send_json_error( __( 'We\'ve detected you\'re requesting with an invalid security token.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
		if( ! isset( $_POST[ 'card' ] ) || ! isset( $_POST[ 'card' ][ 'number' ] ) || ! isset( $_POST[ 'card' ][ 'exp_month' ] ) || ! isset( $_POST[ 'card' ][ 'exp_year' ] ) || ! isset( $_POST[ 'card' ][ 'cvc' ] ) ) {wp_send_json_error( __( 'Some required fields are missing. If you belive it should not to be, please contact to the administrative.',   'iisol-branded-sms-woocommerce-plugin' ), 200 );}
		...
		if( apply_filters( 'futurewordpress/project/payment/stripe/switchpaymentcard', false, $args ) ) {
			wp_send_json_success( __( 'Added Payment Card and is now activated for subscription.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		} else {
			wp_send_json_error( __( 'Something went wrong while trying to update stripe.', 'iisol-branded-sms-woocommerce-plugin' ) );
		}
	}
	public function registerExisting() {
		...
		// 	wp_send_json_error( __( 'We\'ve detected you\'re requesting with an invalid security token or something went wrong with you', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		// }
		if( ! isset( $_POST[ 'userid' ] ) || ! isset( $_POST[ 'password' ] ) || ! isset( $_POST[ 'metadata' ] ) ) {wp_send_json_error( __( 'Something went wrong.',   'iisol-branded-sms-woocommerce-plugin' ), 200 );}
		if( is_array( $_POST[ 'password' ] ) && isset( $_POST[ 'password' ][ 'confirm' ] ) && isset( $_POST[ 'password' ][ 'given' ] ) && $_POST[ 'password' ][ 'given' ] != $_POST[ 'password' ][ 'confirm' ] ) {wp_send_json_error( __( 'Password not matched',   'iisol-branded-sms-woocommerce-plugin' ), 200 );}
		if( ! empty( $_POST[ 'password' ][ 'given' ] ) ) {
			...
			wp_send_json_success( [ 'message' => __( 'Data updated successfully. Please wait a while, we\'re redirecting.', 'iisol-branded-sms-woocommerce-plugin' ), 'redirect' => $redirect, 'hooks' => [ 'register-existing-account-wizard-success' ] ], 200 );
		}
	}
	public function changePassword() {
		...
		if( isset( $password[ 'new' ] ) && ! empty( $password[ 'new' ] ) && isset( $password[ 'old' ] ) && isset( $password[ 'confirm' ] ) && $password[ 'old' ] == $password[ 'confirm' ] ) {
			...
			// wp_send_json_error( __( 'Password Updated Successfully!', 'iisol-branded-sms-woocommerce-plugin' ) );
			set_transient( 'futurewordpress/project/transiant/admin/' . get_current_user_id(), [
				'type'					=> 'success', // primary | danger | success | warning | info
				'message'				=> __( 'Password Updated Successfully!', 'iisol-branded-sms-woocommerce-plugin' )
			], 200 );
		} else {
			// wp_send_json_error( __( 'Password Mismatch. Please try again.', 'iisol-branded-sms-woocommerce-plugin' ) );
			set_transient( 'futurewordpress/project/transiant/admin/' . get_current_user_id(), [
				'type'					=> 'danger', // primary | danger | success | warning | info
				'message'				=> __( 'Password Mismatch. Please try again.', 'iisol-branded-sms-woocommerce-plugin' )
			], 200 );
		}
		wp_redirect( wp_get_referer() );
	}
