 ...
 */
 class Helpers {
	...
	protected function setup_hooks() {
		if( ! defined( 'FUTUREWORDPRESS_PROJECT_BRANDEDSMS_OPTIONS' ) ) {define( 'FUTUREWORDPRESS_PROJECT_BRANDEDSMS_OPTIONS', (array) get_option( 'iisol-branded-sms-woocommerce-plugin', [] ) );}
    ...
    return isset( FUTUREWORDPRESS_PROJECT_BRANDEDSMS_OPTIONS[ $option ] ) ? FUTUREWORDPRESS_PROJECT_BRANDEDSMS_OPTIONS[ $option ] : $default;
	}
	/**
    ...
    return ( isset( FUTUREWORDPRESS_PROJECT_BRANDEDSMS_OPTIONS[ $option ] ) && FUTUREWORDPRESS_PROJECT_BRANDEDSMS_OPTIONS[ $option ] == 'on' );
	}
	/**
	...
	public function uploadFile() {
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'iisol-branded-sms-woocommerce-plugin' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'iisol-branded-sms-woocommerce-plugin' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'iisol-branded-sms-woocommerce-plugin' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'iisol-branded-sms-woocommerce-plugin' );
				wp_send_json_error( $error, 200 );
			}
		}
		wp_send_json_error( __( 'Error happens.', 'iisol-branded-sms-woocommerce-plugin' ) );
	}
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'iisol-branded-sms-woocommerce-plugin' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'iisol-branded-sms-woocommerce-plugin' ), __( '404 not found', 'iisol-branded-sms-woocommerce-plugin' ) );
		}
	}
