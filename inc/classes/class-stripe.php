...
class Stripe {
    ...
    $pay_retainer = get_query_var( 'pay_retainer' );$payment_status = get_query_var( 'payment_status' );
		if ( $pay_retainer && ! empty( $pay_retainer ) && ( $file = FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/cards/pay_retainer.php' ) && file_exists( $file ) && ! is_dir( $file ) ) {
      return $file;
    } else if ( $payment_status && ! empty( $payment_status ) && ( $file = FUTUREWORDPRESS_PROJECT_BRANDEDSMS_DIR_PATH . '/templates/dashboard/cards/payment_status.php' ) && file_exists( $file ) && ! is_dir( $file ) ) {
			return $file;
		} else {
			return $template;
		}
    ...
    $curl = curl_init();
		$param = [
			...
			'line_items'								=> [
				...
				// 			'images' => [ esc_url( FUTUREWORDPRESS_PROJECT_BRANDEDSMS_BUILD_URI . '/icons/Online payment_Flatline.svg' ) ],
				// 		],
				// 	]
				// ]
			],
