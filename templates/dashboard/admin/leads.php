        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'iisol-branded-sms-woocommerce-plugin' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( FUTUREWORDPRESS_PROJECT_BRANDEDSMS_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'iisol-branded-sms-woocommerce-plugin' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'iisol-branded-sms-woocommerce-plugin' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'iisol-branded-sms-woocommerce-plugin' ) : __( 'Pending', 'iisol-branded-sms-woocommerce-plugin' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'iisol-branded-sms-woocommerce-plugin' ) : __( 'Paused', 'iisol-branded-sms-woocommerce-plugin' );
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'iisol-branded-sms-woocommerce-plugin' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
